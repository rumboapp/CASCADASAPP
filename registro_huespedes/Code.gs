...
function obtenerDatosReporte(filtros) {
  ...
  const porPrograma = {};
  filtrados.forEach(r => {
    const prog = idx('Programa') >= 0 ? String(r[idx('Programa')] || '').trim().toUpperCase() : '';
    if (!prog || prog === 'ESTANDAR') porPrograma['ESTANDAR'] = (porPrograma['ESTANDAR'] || 0) + 1;
    else porPrograma[prog] = (porPrograma[prog] || 0) + 1;
  });
